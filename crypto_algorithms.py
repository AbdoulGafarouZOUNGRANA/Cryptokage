...
def des_str_to_bits(s: str, longueur: int) -> list:
    """Convertit une chaîne en liste de bits."""
    bits = []
    for c in s[:longueur]:
        bits.extend([int(b) for b in format(ord(c), '08b')])
    # Padding si nécessaire
    while len(bits) < longueur:
        bits.append(0)
    return bits[:longueur]
...
def des_chiffrer_bloc(bloc_64: list, sous_cles: list) -> list:
    ...
    return des_permuter(R + L, IP_INV)


def des_dechiffrer_bloc(bloc_64: list, sous_cles: list) -> list:
    """Déchiffre un bloc de 64 bits avec DES (sous-clés en ordre inverse)."""
    return des_chiffrer_bloc(bloc_64, sous_cles[::-1])


def des_chiffrer(texte: str, cle: str) -> str:
    """Chiffre un texte avec DES en mode ECB."""
    # Préparation de la clé (64 bits)
...
def cbc_chiffrer(texte: str, cle: str, iv_hex: str = None) -> dict:
    ...
    for i in range(0, len(texte_bytes), 8):
        ...
        bloc_xor = bytes([bloc_clair[j] ^ bloc_precedent[j] for j in range(8)])
        
        # Conversion en bits et chiffrement
        bloc_bits = []
...
def cbc_dechiffrer(chiffre_hex: str, cle: str, iv_hex: str) -> str:
    ...
    """
    cle_bits = des_str_to_bits(cle, 8)
    sous_cles = des_generer_sous_cles(cle_bits)[::-1]
    ...
    for i in range(0, len(chiffre_bytes), 8):
        ...
        # Conversion en bits et déchiffrement
        bloc_bits = []
...
def cfb_chiffrer(texte: str, cle: str, iv_hex: str = None) -> dict:
    """
    Chiffre un texte avec DES en mode CFB.
    ...
    for i in range(0, len(texte_bytes), 8):
        ...
        for octet in registre:
            registre_bits.extend([int(b) for b in format(octet, '08b')])
        ...
        bloc_chiffre = bytes([bloc_clair[j] ^ sortie_bytes[j] for j in range(min(8, len(bloc_clair)))])
        resultat.extend(bloc_chiffre)
        
        # Mise à jour du registre
        registre = list(bloc_chiffre + bytes(8 - len(bloc_chiffre)))
...
def cfb_dechiffrer(chiffre_hex: str, cle: str, iv_hex: str) -> str:
    ...
    for i in range(0, len(chiffre_bytes), 8):
        ...
        for j in range(min(8, len(bloc_chiffre))):
            resultat.append(bloc_chiffre[j] ^ sortie_bytes[j])
